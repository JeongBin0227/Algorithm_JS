...
function solution(N, stages) {
    ...
    let stageNum = new Map()

    stages.forEach(stage => {
        if(stageNum.get(stage)===undefined) stageNum.set(stage,1)
        else stageNum.set(stage,stageNum.get(stage)+1)
    });

    for(let i=1;i<N+1;i++){
        if(stageNum.get(i)!==undefined) {
            tmpAnswer.push([stageNum.get(i)/people,i])
            people -=stageNum.get(i)
        }
        else tmpAnswer.push([0,i])
    }
