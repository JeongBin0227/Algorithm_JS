...
function solution(n, t, m, timetable) {
    ...
    timetable.forEach(tt => {
        let time = 0
        const times = tt.split(':')
        time = parseInt(times[0]*60) + parseInt(times[1])
        sortTimetable.push(time)
    });

    for(let i =0;i<n;i++){
        availableTime.push(540+parseInt(t*i))   
    }

    sortTimetable.sort((a,b)=>{
        return a-b
    })
    ...
    for(let i=0;i<availableTime.length;i++){
        ...
        if(sortTimetable.length!==0){
            if(availableTime[i]>=sortTimetable[0]-1)answer = sortTimetable[0]-1
            else answer =  availableTime[i]
        }
        else{
    ...
    if(2>tmp.length) tmp = 0+tmp
    tmp += ':'
    if(10>answer % 60) tmp = tmp+0
    tmp += answer % 60
